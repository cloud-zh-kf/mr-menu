...
<!--banner end-->
<?php }else{ $tmp_pid=I('get.pid',0); $tmp_ty=I('get.ty',0); $tmp_tty=I('get.tty',0); if($tmp_pid&&$tmp_ty&&$tmp_tty) $myids=$tmp_tty;elseif($tmp_pid&&$tmp_ty&&$tmp_tty==0) $myids=$tmp_ty;if($tmp_pid&&$tmp_ty==0) $myids=$tmp_pid; if($vpid&&$vty&&$vtty) $myidss=$vtty;elseif($vpid&&$vty&&$vtty==0) $myidss=$vty;if($vpid&&$vty==0&&$vtty==0) $myidss=$vpid; if($myids) $myzid=$myids;else $myzid=$myidss; ?>
<div class="nrban">
    <img src="<?php echo get_sort_zd($myzid,'img1');?>">
    <div class="nrbanbox">
        <h3><?php echo get_sort_zd($myzid);?></h3>
        <i></i>
        <p><?php echo get_sort_zd($myzid,'seodescription');?></p>
    </div>
</div>
<?php }?>
<!-- 公司简介 -->
<div class="profilbg">
    ...
    <?php if($vty){?><a href="<?php echo get_nav_url($vpid,$vty);?>" class="on"> <?php echo get_sort_zd($vty);?></a><?php }?>
          ...
          <div class="clearfix profilbox">
               <?php $_result=get_news(1,8,27,0,'*',1);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="profiltit">
                   <img src="images/piotit.png" class="inbk">
                   <span><?php echo ($v["title"]); ?></span>
               </div>
               <div class="profilfl fl">
                    <div class="perfilcet">
                         <?php echo get_html($v['content']);?>
                    </div>
               </div>
               <div class="profilfr fr">
                    <div class="inaboutvoid">
                          <img src="<?php echo get_img($v['img1']);?>">
                          <div class="abvoid pupcu" data-id="account_add"><img src="images/prrofivo.png"></div>
                    </div>
               </div>

               <div class="pup-box" id="account_add">
                   <div class="pup-cell">
                       <div class="retyfee">
                           <div class="rety-close"></div>
                           <video src="<?php echo ($v["file1"]); ?>" controls="" autoplay=""></video>
                       </div>
                   </div>
               </div><?php endforeach; endif; else: echo "" ;endif; ?>
          </div>
...
<div class="bxmbox">
     <ul class="clearfix">
        <?php $_result=get_news(7,28,0,0,'*',1);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li>
           <a>
              <img src="<?php echo get_img($v['img1']);?>">
              <div class="bxmb">
         ...
         <?php $_result=get_news(7,29,0,0,'*',1);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li>
           <a href="">
              <img src="<?php echo get_img($v['img1']);?>">
              <div class="bxmb">
                   <h3><?php echo ($v["title"]); ?></h3>

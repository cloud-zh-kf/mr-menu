                ...
                <div class="fr hearfr">
                    <nav class="nav fl">
                        <ul class="navul">
                            ...
                            <li><a href="<?php echo get_nav_url(5,19);?>" <?php echo get_cur(5,'class="act_nav"');?>>申请入学 </a>
                                <div class="subnav">
                                    <dl class="clearfix">
                                        <?php $_result=get_sort_list(5,10,2);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><dd <?php if($v['id']==$vty) echo 'class="on"';?>>
                                            <a href="<?php echo get_sort_url($v['pid'],$v['id'],$v['linkurl']);?>"><?php echo ($v["catname"]); ?></a>
                                            <div class="thtena">
                                                <?php $_result=get_sort_list($v['id']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?><a href="<?php echo get_sort_urls($v['pid'],$v1['pid'],$v1['id'],$v1['linkurl']);?>"><?php echo ($v1["catname"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                                            </div>
                            ...
                            <li><a href="<?php echo get_nav_url(6,0);?>" <?php echo get_cur(6,'class="act_nav"');?>>加入我们</a>
                                <div class="subnav">
                                    <dl class="clearfix">
                                        <?php $_result=get_sort_list(6,10,6);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><dd <?php if($v['id']==$vty) echo 'class="on"';?>>
                                            <a href="<?php echo get_sort_url($v['pid'],$v['id'],$v['linkurl']);?>"><?php echo ($v["catname"]); ?></a>
                                            <div class="thtena">
                                                <?php $_result=get_sort_list($v['id']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?><a href="<?php echo get_sort_urls($v['pid'],$v1['pid'],$v1['id'],$v1['linkurl']);?>"><?php echo ($v1["catname"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                                            </div>
...
<!--banner end-->
<?php }else{ $tmp_pid=I('get.pid',0); $tmp_ty=I('get.ty',0); $tmp_tty=I('get.tty',0); if($tmp_pid&&$tmp_ty&&$tmp_tty) $myids=$tmp_tty;elseif($tmp_pid&&$tmp_ty&&$tmp_tty==0) $myids=$tmp_ty;if($tmp_pid&&$tmp_ty==0) $myids=$tmp_pid; if($vpid&&$vty&&$vtty) $myidss=$vtty;elseif($vpid&&$vty&&$vtty==0) $myidss=$vty;if($vpid&&$vty==0&&$vtty==0) $myidss=$vpid; if($myids) $myzid=$myids;else $myzid=$myidss; $nr=get_sort_zd($myzid,'seodescription'); ?>
<div class="nrban">
    <img src="<?php echo get_sort_zd($myzid,'img1');?>">
    <div class="nrbanbox">
        <h3><?php echo get_sort_zd($myzid);?></h3>
        <?php if($nr){?>
        <i></i>
        <p><?php echo $nr;?></p>
        <?php }?>
...
<div class="cxjb">
    <?php $_result=get_news(7,12,0,0,'*',1);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="wp">
        <?php echo get_html($v['content']);?>
     </div><?php endforeach; endif; else: echo "" ;endif; ?>
</div>
<?php $_result=get_news(2,13,0,0,'*',10);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="systembox">
     <div class="profiltit">
     ...
     <div class="wp">
         <div class="systemcon">
              <p><?php echo nl2br($v['introduce']);?></p>
              <ul class="clearfix">
                 <li><img src="<?php echo get_img($v['img2']);?>"></li>
                 <li><img src="<?php echo get_img($v['img3']);?>"></li>
                 <li><img src="<?php echo get_img($v['img4']);?>"></li>
              </ul>
         </div>
     </div>
</div><?php endforeach; endif; else: echo "" ;endif; ?>

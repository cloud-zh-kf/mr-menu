...
//ip
function ipchaxun($ipname='1'){
	import('Org.Net.IpLocation');// 导入IpLocation类
	$Ip = new IpLocation(); // 实例化类 参数表示IP地址库文件
	$area = $Ip->getlocation($ipname); // 获取某个IP地址所在的位置
	return $area;
}
...
//获取新闻列表
function get_news_good($pid,$ty,$tty=0,$ttty=0,$zd="*",$num=8,$limit=0,$desc='asc'){
    $str="";
    $q=trim($_GET['q']);
    if($pid) $sqlkey=" AND pid={$pid}";
    if($ty) $sqlkey.=" AND ty={$ty}";
    ...
    $cacheData=false;
    if(!$cacheData){
        $list=D("news")->field($zd)->where("status=1 AND isgood=1{$sqlkey}")->order("id {$desc},disorder desc")->limit($limit,$num)->select();
        S($itemno, $list);
    }else{
        $list=$cacheData;
    }
...
//获取新闻列表
function get_news($pid,$ty,$tty=0,$ttty=0,$zd="*",$num=8,$limit=0,$desc='asc'){
	$str="";
    $q=trim($_GET['q']);
	if($pid) $sqlkey=" AND pid={$pid}";
	if($ty) $sqlkey.=" AND ty={$ty}";
    ...
    $cacheData=false;
	if(!$cacheData){
		$list=D("news")->field($zd)->where("status=1{$sqlkey}")->order("id {$desc},disorder desc")->limit($limit,$num)->select();
 		S($itemno, $list);
	}else{
		$list=$cacheData;
	}
...
//获取字符串分割链接
function get_2explode($string){
 	$str1=explode("\n",$string);
 	$str='';
 	foreach ($str1 as $value) {
 		$str.="<p class='p1'>".$value."</p>";
 	}
  	return $str;
}

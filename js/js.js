...
$(function(){
	...
	$(".menubtn").click(function(){
		...
		if($(this).hasClass('on')){
			...
			$(".inav").stop(false,true).slideUp();
		}else{
			$(this).stop(false,true).addClass("on");
			$(".inav").stop(false,true).slideDown();
		}
  });
	$("body").click(function (e) {
		if (!$(e.target).closest(".inav .wrap").length && !$(e.target).closest(".menubtn").length) {
			$(".inav").hide();
			$(".menubtn").removeClass('on')
		}
		if (!$(e.target).closest(".lanLay").length && !$(e.target).closest(".curLan").length) {
			$(".lanLay").hide();
			$(".curLan").removeClass('on')
		}
	});
...
$window.scroll(function() {
  ...
  //javascript用offsetTop();jquery用offset().top;
	...
	$(".joindl dt").click(function(){
		...
		$('#wtfk').css('display','block');
		$('#yjfk').css('display','none');
		$('.wtfk').val($(this).attr('act'));
		})
	...
	function f() {
		...
		if(_width<1024){
			$('.curLan').click(function(){			
				if($(this).hasClass('on')){
					$(this).siblings('.lanLay').stop(false,true).slideUp();
					$(this).removeClass('on')
				}else{
					$(this).siblings('.lanLay').stop(false,true).slideDown();
					$(this).addClass('on')
				}
				
			})
			$(window).scroll(function(){
				$('.lanLay').stop(false,true).slideUp();
				$('.curLan').removeClass('on')
			})

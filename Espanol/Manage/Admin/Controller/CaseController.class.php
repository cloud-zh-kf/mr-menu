...
class CaseController extends PurviewController {
    ...
    public function Index(){
        ...
        $vs['ttty']=$ttty;
        $vs['zid']=$zid;
        $vs['listurl']=U('casexq/List',"pid={$pid}&ty={$ty}&tty={$tty}&ttty={$ttty}");
        $vs['addurl']=U('casexq/Add',"pid={$pid}&ty={$ty}&tty={$tty}&ttty={$ttty}");
        ...
        $list = $db->where($sqlkey)->order("istop desc,disorder desc,id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('vs',$vs);
        ...
        if(IS_POST){
            ...
            if($act=='sorting'){
                for($i=0;$i<count($checkid);$i++) {
                    D()->execute("update __PREFIX__casexq set disorder=$sorting_arr[$i] where id='{$checkid[$i]}'");
                    sys_log("信息排序更新:(id:$checkid[$i])");
                }
                $this->success("信息排序更新成功",cookie('Jumpurl'));
            }elseif($act=='alldel') {
                for($i=0;$i<count($checkid);$i++) {
                    D()->execute("delete from  __PREFIX__casexq where id='{$checkid[$i]}'");

...
KindEditor.ready(function(K) {
	...
	<?php $__FOR_START_30061__=1;$__FOR_END_30061__=$imgnum;for($i=$__FOR_START_30061__;$i < $__FOR_END_30061__;$i+=1){ ?>K('#img<?php echo ($i); ?>_btn').click(function() {
...
<body>
	...
	<ul class="forminfo">
		...
		<?php if($v['pid']==2){?>
 		<li><label>所属分类<b>*</b></label>
			<select name="ty" id="pid" >
				...
				<?php $_result=get_table_list('sort',' AND pid=2','pid,id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><option value="<?php echo ($s["id"]); ?>"><?php echo ($s["catname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>

			<select name="tty" id="ty" style="display: none">
				<option value="0">请选择三级</option>
			</select>

			<select name="ttty" id="tty" style="display: none">
		...
		<?php if(in_array(6,$arr)): ?><li><label>摘要<b>*</b></label><textarea name="introduce" style="width:520px;height:150px;" class="dfinput"><?php echo ($v["introduce"]); ?></textarea></li><?php endif; ?>
		...
		<?php if(in_array(10,$arr)): $__FOR_START_26018__=1;$__FOR_END_26018__=$imgnum;for($i=$__FOR_START_26018__;$i < $__FOR_END_26018__;$i+=1){ ?><li><label><?php echo get_imgname($i,$v['pid'],$v['ty'],$v['tty']);?><b>*</b></label><input type="text" name="img<?php echo ($i); ?>" id="img<?php echo ($i); ?>" value="" class="dfinput"/> <input type="button"

	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		...
		<div class="layui-tab-content">
			<div class="layui-tab-item layui-show">
				<form action="/Japanese/jxadmin.php/Admin-News-Index" method="get" class="layui-form">
				...
				<form action="<?php echo U('News/Action');?>" method="post" id="contentForm" name="formlist" class="layui-form" onkeydown="if(event.keyCode==13) return false;">
				<table class="layui-table">
					...
					<tr>
						...
						<!-- <?php if($vs['pid'] == 3): ?><th>案例详情内容</th><?php endif; ?> -->
						<th>排序</th>
						<th>状态</th>
						<th>发布时间</th>
						<th>操作</th>
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<!-- <?php if($vs['pid'] == 3): ?><td><a href='Admin-Case-Index-pid-<?php echo ($v["id"]); ?>-ty-<?php echo ($v["ty"]); ?>-tty-<?php echo ($v["tty"]); ?>.html'  class='layui-btn layui-btn-xs'>修改</a></td><?php endif; ?> -->
							<td class="table-input">
 								<input type="text" name="sorting[]" value="<?php echo ($v['disorder']); ?>" class="layui-input">
							</td>
							<td>
								<?php if($v['status']==1){?>
								<a href='<?php echo get_url($v['id'],'News','Audit');?>'><i class='fa fa-toggle-on' title="点击关闭"></i></a>
								<?php }else{?>
								<a href='<?php echo get_url($v['id'],'News','Audit');?>'><i class='fa fa-toggle-off' title="点击开启"></i></a>
								<?php }?>
							</td>
							<td><?php echo get_time($v['sendtime'],'Y-m-d');?></td>
							<td>
								<a href='<?php echo get_url($v['id'],'News','Add');?>'  class='layui-btn layui-btn-xs'>修改</a>
								<a href='<?php echo get_url($v['id'],'News','Del');?>' onclick='return confirm("您确定要删除吗？\n\n删除后不可恢复!")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
							</td>

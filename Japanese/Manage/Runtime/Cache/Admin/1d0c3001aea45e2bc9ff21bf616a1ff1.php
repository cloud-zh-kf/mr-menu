...
KindEditor.ready(function(K) {
	...
	<?php $__FOR_START_7527__=1;$__FOR_END_7527__=$imgnum;for($i=$__FOR_START_7527__;$i < $__FOR_END_7527__;$i+=1){ ?>K('#img<?php echo ($i); ?>_btn').click(function() {
...
<body>
    ...
    <div id="usual1" class="usual"> 
		...
		<?php if(get_count("sort","pid>0 AND  pid={$v['tty']}")>0){?>
 		<li><label>所属分类<b>*</b></label>
			<cite>
				<select name="ty" id="pid" >
					<option value="0">请选择二级</option>
					<?php $_result=get_table_list('sort',' AND pid=2','pid,id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><option value="<?php echo ($s["id"]); ?>" <?php if($s['id'] == $v['ty']): ?>selected<?php endif; ?>><?php echo ($s["catname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>

				<select name="tty" id="ty" >
					<option value="0">请选择三级</option>

					<?php $_result=get_table_list('sort',' AND pid='.$v['ty'].'','pid,id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><option value="<?php echo ($s["id"]); ?>" <?php if($s['id'] == $v['tty']): ?>selected<?php endif; ?>><?php echo ($s["catname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>


 				</select>

				<select name="ttty" id="tty">
				<option value="0">请选择四级</option>

					<?php $_result=get_table_list('sort',' AND pid='.$v['tty'].'','pid,id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><option value="<?php echo ($s["id"]); ?>" <?php if($s['id'] == $v['ttty']): ?>selected<?php endif; ?>><?php echo ($s["catname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		...
		<li><label>学校名称<b>*</b></label><input name="title" type="text" class="dfinput" value="<?php echo ($v["title"]); ?>" style="width:518px;"/>
		...
		<?php if(in_array(2,$arr)): ?><li><label>占地面积<b>*</b></label><input name="tags" type="text" value="<?php echo ($v["tags"]); ?>" class="dfinput"> M2</li><?php endif; ?>

		<?php if(in_array(12,$arr)): ?><li><label>建筑面积<b>*</b></label><input name="source" type="text" value="<?php echo ($v["source"]); ?>" class="dfinput"> M2</li><?php endif; ?>

		<?php if(in_array(13,$arr)): ?><li><label>教育配套设施<b>*</b></label><input name="author" type="text" value="<?php echo ($v["author"]); ?>" class="dfinput"> 间</li><?php endif; ?>
		...
		<?php if($v['pid'] == 2): ?><li><label>产品参数<b>*</b></label><textarea name="introduce" style="width:520px;height:100px;" class="dfinput"><?php echo ($v["introduce"]); ?></textarea>
				多个参数用回车分割，如电源：+10~30V，回车 最大径向受力：60N
			</li><?php endif; ?>
		...
		<?php if(in_array(9,$arr)): ?><li><label>学校简介<b>*</b></label><textarea class="editor_id" name="content" style="width:667px;height:350px;"><?php echo ($v["content"]); ?></textarea></li><?php endif; ?>
		
		<?php if(in_array(10,$arr)): $__FOR_START_5908__=1;$__FOR_END_5908__=$imgnum;for($i=$__FOR_START_5908__;$i < $__FOR_END_5908__;$i+=1){ $zd="img$i"; ?>
		...
		<?php if(in_array(16,$arr)): ?><li><label>多图上传<b>*</b></label><input type="button" id="images" value="批量上传" class="btn"/> 图片尺寸:391 x 258<br>
				<div id="imagesview">

...
<div class="layui-side layui-bg-black">
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		<ul class="layui-tab-title">
			<li class="layui-this"><a href="<?php echo U('Manager/Index');?>">系统用户列表</a></li>
			<li><a href="<?php echo U('Manager/Add');?>">系统用户添加</a></li>
		</ul>
			...
			<div class="layui-tab-item layui-show">
				<table class="layui-table">
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo get_role_zd($v['role_id']);?></td>
							<td><?php echo ($v['realname']); ?></td>
							<td><?php echo ($v['username']); ?></td>
							<td>
								<?php if($v['status']==1){?>
								<a href='<?php echo get_url($v['id'],'Manager','Audit');?>'><i class='fa fa-toggle-on' title="点击关闭"></i></a>
								<?php }else{?>
								<a href='<?php echo get_url($v['id'],'Manager','Audit');?>'><i class='fa fa-toggle-off' title="点击开启"></i></a>
								<?php }?>
							</td>
							<td>
								<a href='<?php echo get_url($v['id'],'Manager','Add');?>'  class='layui-btn layui-btn-xs'>修改</a>
								<?php if($v['id']>1){?>
								<a href='<?php echo get_url($v['id'],'Manager','Del');?>' onclick='return confirm("您确定要删除吗？")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
								<?php }?>
							</td>

...
<div class="layui-side layui-bg-black">
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		<ul class="layui-tab-title">
			<li class="layui-this"><a href="<?php echo U('Forms/Index');?>"><?php echo get_forms_zd(I('request.typeid','title'))?>表管理</a></li>
			<li><a href="<?php echo U(CONTROLLER_NAME.'/FieldAdd',"typeid=".I('request.typeid')."");?>">字段添加</a></li>
		</ul>
			...
			<div class="layui-tab-item layui-show">
				<table class="layui-table">
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php if($v['Field']<>"id"&&$v['Field']<>"ip"&&$v['Field']<>"status"&&$v['Field']<>"sendtime"){?>
								<a href="<?php echo U(CONTROLLER_NAME.'/FieldAdd',"act=up&zd=".$v['Field']."&title=".$v['title']."&tip=".$v['tip']."&zd=".$v['Field']."&typeid=".I('request.typeid')."");?>"  class='layui-btn layui-btn-xs'>修改</a>
								<a href="<?php echo U(CONTROLLER_NAME.'/FieldDel',"zd=".$v['Field']."&typeid=".I('request.typeid')."");?>" onclick='return confirm("您确定要删除模块么？")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
								<?php }else{?>
								--
								<?php }?>
							</td>

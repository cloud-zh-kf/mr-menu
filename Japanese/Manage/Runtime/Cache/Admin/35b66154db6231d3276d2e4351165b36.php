    ...
    <div class="rightinfo">
		...
		<table class="tablelist">
			<thead>
				...
				<th width="50px">选择</th>
				<th width="50px">序号</th>
				<th width="80px">专辑／视频</th>
				<th width="80px">订阅用户</th>
				<th width="80px">被订阅模特</th>
				<th width="120px">流水号</th>
				<th width="80px">订阅类型</th>
				<th width="50px">数量</th>
				<th width="50px">大小</th>
				<th width="50px">虚拟币</th>
				<th width="250px">备注</th>
				<th width="50px">状态</th>
				<th width="140px">订阅时间</th>
				</tr>
			</thead>
			<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			<td><input name="checkid[]" type="checkbox" value="<?php echo ($v["cid"]); ?>"></td>
			<td><?php echo ($v["id"]); ?></td>
			<td><img src="<?php echo ((isset($v["img1"]) && ($v["img1"] !== ""))?($v["img1"]):'/Uploads/nopic.jpg'); ?>" width="80" /></td>
			<td><?php echo get_user_zd($v['uid']);?></td>
			<td><?php echo get_user_zd($v['puid']);?></td>
			<td><?php echo ($v["orderid"]); ?></td>
			<td><?php if($v['typeid'] == 0): ?>普通<?php else: ?>VIP<?php endif; ?>订阅</td>
			<td><?php echo get_count("photo","pid=".$v['id']." AND uid=".$v['uid']."");?>P</td>
			<td><?php echo ($v["filesize"]); ?> M</td>
			<td><?php echo ($v["price"]); ?>币</td>
			<td><?php echo ($v["remark"]); ?></td>
			<td>
			<?php if(($v['ispay']) == "1"): ?><a href="<?php echo U(CONTROLLER_NAME.'/Audit',"id=".$v['cid']."");?>"><img width="16" height="16" border="0" alt="已启用" src="<?php echo (ADMIN_PUBLIC); ?>/images/p.png"/></a>
			<?php else: ?>
			<a href="<?php echo U(CONTROLLER_NAME.'/Audit',"id=".$v['cid']."");?>"><img width="16" height="16" border="0" alt="已查封" src="<?php echo (ADMIN_PUBLIC); ?>/images/x.png"/></a><?php endif; ?>			</td>

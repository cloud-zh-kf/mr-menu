        ...
        <li class="layui-nav-item layui-hide-xs">
            <a href="javascript:;">
                ...
                <dl class="layui-nav-child">
                    <?php if($_SESSION['admin_id']== 99999999): ?><dd ><a href="<?php echo U('Settings/Index');?>"><i class="fa fa-sliders" aria-hidden="true"></i>系统设置</a></dd>
                    <dd ><a href="<?php echo U('Model/Index');?>"><i class="fa fa-codepen" aria-hidden="true"></i>模型管理</a></dd>
                    ...
                    <dd ><a href="<?php echo U('Backup/Index');?>"><i class="fa fa-database" aria-hidden="true"></i>数据备份</a></dd><?php endif; ?>
...
<div class="layui-side layui-bg-black">
    <div class="layui-side-scroll">
        ...
        <ul class="layui-nav layui-nav-tree" id="nav" lay-shrink="all">
            <?php $_result=get_mytable_list('sort',' AND pid=0','id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?><li class="layui-nav-item nav-item<?php if($v1['id']==I('get.pid',0)){?> layui-nav-itemed<?php }?>">
                ...
                <?php $_result=get_mytable_list('sort',' AND pid='.$v1['id'].'','id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($i % 2 );++$i; $count2=get_count("sort",'status=1 AND pid='.$v2['id'].''); if($count2) $url2="javascript:;";else $url2=U("{$v2['sort_c']}/{$v2['sort_a']}","pid=".$v1['id']."&ty=".$v2['id'].""); ?>
                    <dl class="layui-nav-child ">
                        ...
                        <?php if($count2){?>
                            <dl class="layui-nav-child">
                                <dd <?php if($v3['id']==I('get.tty',0)){?>class="layui-nav-itemed"<?php }?>>
                                    <?php $_result=get_mytable_list('sort',' AND pid='.$v2['id'].'','id,catname,sort_a,sort_c');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v3): $mod = ($i % 2 );++$i;?><a href="<?php echo U("{$v3['sort_c']}/{$v3['sort_a']}","pid=".$v1['id']."&ty=".$v2['id']."&tty=".$v3['id']."")?>"><i class="fa fa-file-text-o" aria-hidden="true"></i><?php echo ($v3['catname']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                                </dd>
                            </dl>
                        <?php }?>
                        </dd>
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		<ul class="layui-tab-title">
			<li class="layui-this"><a href="<?php echo U('Model/Index');?>">模型列表</a></li>
			<li><a href="<?php echo U('Model/Add');?>">模型添加</a></li>
		</ul>
			...
			<div class="layui-tab-item layui-show">
				<table class="layui-table">
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php if($v['status']==1){?>
								<a href='<?php echo get_url($v['id'],'Model','Audit');?>'><i class='fa fa-toggle-on' title="点击关闭"></i></a>
								<?php }else{?>
								<a href='<?php echo get_url($v['id'],'Model','Audit');?>'><i class='fa fa-toggle-off' title="点击开启"></i></a>
								<?php }?>
							</td>
							<td>
								<a href='<?php echo get_url($v['id'],'Model','Add');?>'  class='layui-btn layui-btn-xs'>修改</a>
								<a href='<?php echo get_url($v['id'],'Model','Del');?>' onclick='return confirm("您确定要删除模块么？")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
							</td>

...
<script>
    ...
    <?php $__FOR_START_1525982198__=1;$__FOR_END_1525982198__=$v['imgnum'];for($i=$__FOR_START_1525982198__;$i < $__FOR_END_1525982198__;$i+=1){ ?>K('#img<?php echo ($i); ?>_btn').click(function() {
    ...
    var editor = K.editor({
	    ...
	    <?php $__FOR_START_691808429__=1;$__FOR_END_691808429__=C(BANNER_NUM)+1;for($i=$__FOR_START_691808429__;$i < $__FOR_END_691808429__;$i+=1){ ?>K('#img<?php echo ($i); ?>_btn').click(function() {
	            editor.loadPlugin('image', function() {
	                editor.plugin.imageDialog({
	                    imageUrl : K('#img<?php echo ($i); ?>').val(),
	                    clickFn : function(url, title, width, height, border, align) {
	...
	<div class="layui-body">
		<div class="layui-tab layui-tab-brief" lay-filter="tab">
			<ul class="layui-tab-title">
				<li><a href="<?php echo U('Sort/Index');?>">栏目列表</a></li>
				<li class="layui-this"><a href="<?php echo U('Sort/Add');?>">栏目添加</a></li>
			</ul>
			<div class="layui-tab-content">
				<div class="layui-tab-item layui-show">
					<form action="" method="post" class="layui-form" onSubmit="return check_manager_add(this);">
						<input type="hidden" name="id" value="<?php echo ($v['id']); ?>">
						<?php if($_SESSION['admin_id']== 99999999): ?><div class="layui-form-item">
							...
							<div class="layui-form-item">
								...
								<div class="layui-input-inline">
									<select name="model_id" lay-filter="model" required lay-verify="required">
										...
										<?php $_result=get_mytable_list('model');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><option value="<?php echo ($s["id"]); ?>|<?php echo get_model_zd($s['id'],'model_c');?>|<?php echo get_model_zd($s['id'],'model_a');?>" <?php if($s['id'] == $v['model_id']): ?>selected<?php endif; ?>><?php echo ($s["model_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							</div>
						...
						<?php else: ?>
							...
							<?php if(!$v['pid']): ?><input type="hidden" name="model_id" value="<?php echo get_sort_zd($v['pid'],'model_id');?>">
								<?php else: ?>
								<input type="hidden" name="model_id" value="<?php echo ($v["model_id"]); ?>"><?php endif; ?>
						...
						<div class="layui-form-item">
								...
								<div class="layui-input-block">
									<textarea class="editor_id" name="introduce" style="width:667px;height:350px;"><?php echo ($v["introduce"]); ?></textarea>                            </div>
							</div>
						...
						<?php if($_SESSION['admin_id']== 99999999): ?><div class="layui-form-item">
							...
							<div class="layui-form-item">
								...
								<div class="layui-input-inline">
									<input type="text" name="sort_url" id="sort_url" value="<?php echo ($v['sort_url']); ?>" placeholder="请输入内部自定义链接" class="layui-input">
								</div>
							</div>
						...
						<?php else: ?>
							...
							<input type="hidden" name="isopen" value="<?php echo get_sort_zd($v['pid'],'isopen');?>"><?php endif; ?>



						<?php $__FOR_START_653204766__=1;$__FOR_END_653204766__=C(BANNER_NUM)+1;for($i=$__FOR_START_653204766__;$i < $__FOR_END_653204766__;$i+=1){ $zd="img$i";?>
							<div class="layui-form-item">

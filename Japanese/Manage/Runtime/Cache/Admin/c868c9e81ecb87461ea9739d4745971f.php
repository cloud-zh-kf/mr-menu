...
<div class="layui-side layui-bg-black">
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		<ul class="layui-tab-title">
			<li class="layui-this"><a href="Admin-Case-Index-pid-<?php echo ($vs["pid"]); ?>-ty-<?php echo ($vs["ty"]); ?>-tty-<?php echo ($vs["tty"]); ?>">案例详情内容</a></li>
			<li><a href="Admin-Case-Add-pid-<?php echo ($vs["pid"]); ?>-ty-<?php echo ($vs["ty"]); ?>-tty-<?php echo ($vs["tty"]); ?>">案例详情内容添加</a></li>
		</ul>
		<div class="layui-tab-content">
			<div class="layui-tab-item layui-show">
				<form action="/Admin-Case-Index" method="get" class="layui-form">
					...
					<!-- <input type="hidden" name="uid" value="<?php echo ($vs["uid"]); ?>"> -->
					<div class="layui-form-item nospace">
						...
						<div class="layui-input-inline">
							...
							<a class="layui-btn layui-btn-primary"  href="">清除搜索</a>
						</div>
					</div>
				</form>

				<form action="<?php echo U('Case/Action');?>" method="post" id="contentForm" name="formlist" class="layui-form" onkeydown="if(event.keyCode==13) return false;">
				<table class="layui-table">
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($v["title"]); ?> <?php if($v['isgood'] == 1): ?><font color="red">推荐</font><?php endif; ?> <?php if($v['istop'] == 1): ?><font color="blue">置顶</font><?php endif; ?></td>
							<td class="table-input">
 								<input type="text" name="sorting[]" value="<?php echo ($v['disorder']); ?>" class="layui-input">
							</td>
							<td>
								<?php if($v['status']==1){?>
								<a href='<?php echo get_url($v['id'],'Case','Audit');?>'><i class='fa fa-toggle-on' title="点击关闭"></i></a>
								<?php }else{?>
								<a href='<?php echo get_url($v['id'],'Case','Audit');?>'><i class='fa fa-toggle-off' title="点击开启"></i></a>
								<?php }?>
							</td>
							<td><?php echo get_time($v['sendtime'],'Y-m-d');?></td>
							<td>
								<a href='<?php echo get_url($v['id'],'Case','Add');?>'  class='layui-btn layui-btn-xs'>修改</a>
								<a href='<?php echo get_url($v['id'],'Case','Del');?>' onclick='return confirm("您确定要删除吗？\n\n删除后不可恢复!")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
							</td>

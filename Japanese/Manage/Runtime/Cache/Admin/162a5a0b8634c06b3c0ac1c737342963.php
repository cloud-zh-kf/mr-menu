            ...
            <a href="javascript:;">
                ...
                <dl class="layui-nav-child">
                    ...
                    <?php else: ?>
                        ...
                        <?php if(in_array(9906,$qxid)): ?><dd ><a href="<?php echo U('Logs/Index');?>"><i class="fa fa-history" aria-hidden="true"></i>系统日志</a></dd>
                        <if condition="in_array(9908,$qxid)"><dd ><a href="<?php echo U('Sort/Make');?>"><i class="fa fa-database" aria-hidden="true"></i>更新缓存</a></dd><?php endif; endif; ?>
            ...
            <?php $_result=get_mytable_list('sort',' AND pid=0','id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i;?><li class="layui-nav-item nav-item<?php if($v1['id']==I('get.pid',0)){?> layui-nav-itemed<?php }?>">
                ...
                <?php $_result=get_mytable_list('sort',' AND pid='.$v1['id'].'','id,catname');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($i % 2 );++$i; $count2=get_count("sort",'status=1 AND pid='.$v2['id'].''); if($count2) $url2="javascript:;";else $url2=U("{$v2['sort_c']}/{$v2['sort_a']}","pid=".$v1['id']."&ty=".$v2['id'].""); ?>
                    <dl class="layui-nav-child ">
                        ...
                        <?php if($count2){?>
                            <?php $_result=get_mytable_list('sort',' AND pid='.$v2['id'].'','id,catname,sort_a,sort_c');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v3): $mod = ($i % 2 );++$i;?><dl class="layui-nav-child">
	...
	<div class="layui-body">
		<div class="layui-tab layui-tab-brief" lay-filter="tab">
			<ul class="layui-tab-title">
				<li><a href="<?php echo U('Manager/Index');?>">系统用户列表</a></li>
				<li class="layui-this"><a href="<?php echo U('Manager/Add');?>">系统用户添加</a></li>
			</ul>
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<select name="role_id" lay-filter="model" lay-verify="required">
									...
									<?php $_result=get_table_list('role','','id,role_name');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vs): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vs["id"]); ?>" <?php if($vs['id'] == $v['role_id']): ?>selected<?php endif; ?>><?php echo ($vs["role_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
								</select>
							</div>
						</div>

...
<div class="layui-side layui-bg-black">
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
		<ul class="layui-tab-title">
			<?php if($_SESSION['admin_id']== 99999999): ?><li class="layui-this"><a href="<?php echo U('Forms/Index');?>">表单管理</a></li>
				<li><a href="<?php echo U(CONTROLLER_NAME.'/InfoAdd',"typeid=".I('request.typeid')."");?>">数据添加</a></li>
				<?php else: ?>
				<li class="layui-this"><a href="javascript:;">表单管理</a></li><?php endif; ?>
		</ul>
		<div class="layui-tab-content">
			<div class="layui-tab-item layui-show">
				<form action="<?php echo U('Forms/InfoDel');?>" method="post" id="contentForm" name="formlist" class="layui-form" onkeydown="if(event.keyCode==13) return false;">
				<input type="hidden" name="typeid" value="<?php echo I('request.typeid');?>">
				<table class="layui-table">
					<thead>
					<tr>
						<th>选择</th>

						<?php $_result=get_myfields_list(I('request.typeid'));if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vs): $mod = ($i % 2 );++$i;?><th><?php echo ($vs["title"]); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
						<th>操作</th>
					</tr>
					</thead>
					<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<?php $_result=get_myfields_list(I('request.typeid'));if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vs): $mod = ($i % 2 );++$i;?><td><?php echo ($v[$vs['Field']]); ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
							<td>
								<a href='jxadmin.php?m=Admin&c=Forms&a=Del&id=<?php echo ($v["id"]); ?>&typeid=<?php echo I("request.typeid");?>' onclick='return confirm("您确定要删除吗？\n\n删除后不可恢复!")' class='layui-btn layui-btn-xs layui-btn-danger' title='删除'>删除</a>
							</td>
